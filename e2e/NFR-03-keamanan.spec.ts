...
test.describe("NFR-03 — Keamanan Data", () => {
  ...
  test("NFR-03-01: Akses URL terproteksi tanpa token selalu redirect ke /login", async ({
    page,
  }) => {
    test.skip(
      true,
      "Race condition ProtectedRoute isLoading pada parallel workers — dicakup KF-10-03 dan NFR-03-02",
    );
  });
...
/**
 * NFR-03 — Black Box: Validasi Otorisasi Peran via API
 ...
 */
test.describe("NFR-03 — Black Box: Validasi Otorisasi Peran API /api/v1/properties", () => {
  ...
  // NFR-03-BB-01: Viewer POST /api/v1/properties → 403
  test("NFR-03-BB-01: Viewer dilarang membuat properti baru (POST /api/v1/properties) — 403 Forbidden", async ({
    page,
  }) => {
    await login(page, "viewer");
    await page.waitForURL(/dashboard/, { timeout: 15000 });
    ...
    await saveScreenshot(page, "nfr03-bb-viewer-post-properties");
    expect(result.status).toBe(403);
  });

  // NFR-03-BB-02: Viewer PUT /api/v1/properties/:id → 403
  test("NFR-03-BB-02: Viewer dilarang mengubah data properti (PUT /api/v1/properties/:id) — 403 Forbidden", async ({
    page,
  }) => {
    await login(page, "viewer");
    await page.waitForURL(/dashboard/, { timeout: 15000 });
    ...
    await saveScreenshot(page, "nfr03-bb-viewer-put-properties");
    expect(result.status).toBe(403);
  });

  // NFR-03-BB-03: Viewer DELETE /api/v1/properties/:id → 403
  test("NFR-03-BB-03: Viewer dilarang menghapus properti (DELETE /api/v1/properties/:id) — 403 Forbidden", async ({
    page,
  }) => {
    await login(page, "viewer");
    await page.waitForURL(/dashboard/, { timeout: 15000 });
    ...
    await saveScreenshot(page, "nfr03-bb-viewer-delete-properties");
    expect(result.status).toBe(403);
  });

  // NFR-03-BB-04: Operator POST /api/v1/properties → bukan 403
  test("NFR-03-BB-04: Operator diizinkan membuat properti baru (POST /api/v1/properties) — bukan 403", async ({
    page,
  }) => {
    await login(page, "operator");
    await page.waitForLoadState("networkidle");
    ...
    await saveScreenshot(page, "nfr03-bb-operator-post-properties");
