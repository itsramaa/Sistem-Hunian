...
test.describe("Dashboard - Operator", () => {
  ...
  test("AC-DASH-01: Summary cards tampil lengkap", async ({ page }) => {
    ...
    await saveScreenshot(page, "dashboard-operator-desktop");
  ...
  test("AC-DASH-07: Dark mode readable", async ({ page }) => {
    ...
    const themeBtn = page.getByRole("button", { name: /theme|dark|light|mode/i });
    ...
    await saveScreenshot(page, "dashboard-operator-dark");
  });

  test("AC-DASH-08: Light mode readable", async ({ page }) => {
    await page.waitForLoadState("networkidle");
    await page.evaluate(() => document.documentElement.classList.remove("dark"));
    await saveScreenshot(page, "dashboard-operator-light");
  });

  test("AC-DASH-09: Mobile responsive 375px", async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 812 });
    await page.waitForLoadState("networkidle");
    await saveScreenshot(page, "dashboard-operator-mobile");
...
test.describe("Dashboard - Manager", () => {
  ...
  test("AC-DASH-04: Manager melihat dashboard", async ({ page }) => {
    await page.waitForLoadState("networkidle");
    await saveScreenshot(page, "dashboard-manager-desktop");
...
test.describe("Dashboard - Viewer", () => {
  ...
  test("AC-DASH-05: Viewer hanya melihat summary cards", async ({ page }) => {
    await page.waitForLoadState("networkidle");
    await saveScreenshot(page, "dashboard-viewer-desktop");
    expect(page.url()).toContain("/dashboard");
  });
  ...
  test("AC-DASH-09: Viewer mobile 375px", async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 812 });
    await page.waitForLoadState("networkidle");
    await saveScreenshot(page, "dashboard-viewer-mobile");
  });
});
