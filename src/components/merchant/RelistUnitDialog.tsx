...
export function RelistUnitDialog({ 
  ...
  const rentChange = monthlyRent > 0 && unit.monthly_rent > 0
    ? ((monthlyRent - unit.monthly_rent) / unit.monthly_rent * 100).toFixed(1)
    : 0;

  function generateDescription(unit: any) {
    const features = [];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to list unit");
    } finally {
