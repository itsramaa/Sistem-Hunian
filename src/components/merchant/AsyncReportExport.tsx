...
interface AsyncReportExportProps {
  onExport: (type: string, params: any) => Promise<ExportJob>;
  className?: string;
}
...
export function useAsyncExport() {
  ...
  const startExport = async (
    type: string, 
    params: any, 
    exportFn: (type: string, params: any) => Promise<{ data: any[]; filename: string }>
  ) => {
    ...
    try {
      ...
      setJobs(prev => prev.map(j => 
        j.id === jobId 
          ? { ...j, status: 'failed', error: error.message || 'Export failed' } 
          : j
      ));
  ...
  const retryJob = (jobId: string, exportFn: (type: string, params: any) => Promise<{ data: any[]; filename: string }>) => {
