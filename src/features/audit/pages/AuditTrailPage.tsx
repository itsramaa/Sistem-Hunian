...
export default function AuditTrailPage() {
  ...
  const fmt = (d: string) => { try { return format(new Date(d), 'dd MMM yyyy HH:mm', { locale: localeId }); } catch { return d; } };
            ...
            <TableBody>
              {logs.length === 0 ? (
                ...
                <TableRow key={log.id} className="hover:bg-primary/5 transition-colors">
                  ...
                  <TableCell>
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${statusColors[log.old_status] ?? ''}`}>
                      {log.old_status}
                    </span>
                  </TableCell>
                  <TableCell>
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${statusColors[log.new_status] ?? ''}`}>
                      {log.new_status}
                    </span>
                  </TableCell>
