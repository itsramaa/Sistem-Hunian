        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
          <div>
            ...
            {form.formState.errors.unit_id && (
              <p className="text-sm text-destructive mt-1">{form.formState.errors.unit_id.message}</p>
            )}
          </div>
          <div>
            <Label>Select Tenant</Label>
            <Select 
              value={form.watch('tenant_user_id')} 
              onValueChange={(v) => form.setValue('tenant_user_id', v, { shouldValidate: true })}
            >
             ...
             {form.formState.errors.tenant_user_id && (
              <p className="text-sm text-destructive mt-1">{form.formState.errors.tenant_user_id.message}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
                ...
                <Input type="number" {...form.register('rent_amount')} />
            ...
            <div>
                ...
                <Input type="number" {...form.register('deposit_amount')} />
                {form.formState.errors.deposit_amount && (
                  <p className="text-sm text-destructive mt-1">{form.formState.errors.deposit_amount.message}</p>
                )}
            </div>
          </div>
          <div>
            <Label>Terms & Conditions</Label>
            <Textarea placeholder="Contract terms..." {...form.register('terms')} rows={3} />
