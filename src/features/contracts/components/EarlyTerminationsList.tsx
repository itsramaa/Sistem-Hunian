  ...
  if (requests.length === 0) {
    return (
      <EmptyState
        ...
        description="Semua permintaan pemutusan awal telah diproses."
      />
    );
  }
          ...
          <CardHeader>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-lg">Permintaan Pemutusan Awal</CardTitle>
                  <CardDescription>{request.contract?.unit?.property?.name} - Unit {request.contract?.unit?.unit_number}</CardDescription>
                </div>
              </div>
              <Badge variant="secondary" className="rounded-full">Menunggu Persetujuan</Badge>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
              {[
                { label: "Tanggal Diajukan", value: format(new Date(request.requested_date), "dd MMM yyyy", { locale: id }) },
                { label: "Jumlah Penalti", value: formatCurrency(request.penalty_amount), className: "text-destructive" },
                { label: "Alasan", value: request.reason || "Tidak ditentukan" },
              ].map((item, i) => (
