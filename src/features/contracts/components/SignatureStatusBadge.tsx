...
export function SignatureStatusBadge({ contract }: SignatureStatusBadgeProps) {
  ...
  if (contract.tenant_signature_url && !contract.merchant_signature_url) {
    return <Badge variant="outline" className="gap-1 text-warning border-warning rounded-full"><PenLine className="h-3 w-3" /> Awaiting Your Signature</Badge>;
  }
  return <Badge variant="outline" className="gap-1 rounded-full"><PenLine className="h-3 w-3" /> Pending</Badge>;
}
