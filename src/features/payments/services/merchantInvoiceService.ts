...
import { INVOICE_STATUS_TRANSITIONS, isValidTransition } from '@/shared/constants/state-machines';
...
export const merchantInvoiceService = {
  ...
  async createInvoice(payload: CreateInvoicePayload): Promise<void> {
    ...
    const { error } = await supabase
      .from('invoices')
      .insert({
        invoice_number: '', // Auto-generated by DB trigger generate_invoice_number()
        contract_id,
        merchant_id,
        tenant_user_id,
        amount,
    ...
    await createAuditLog({
      action: 'create',
      entityType: 'invoice',
      newData: { contract_id, merchant_id, tenant_user_id, amount, due_date },
    });
  },
  ...
  async sendInvoice(invoiceId: string, merchantName: string): Promise<void> {
    ...
    // Validate transition: draft → sent
    if (!isValidTransition(INVOICE_STATUS_TRANSITIONS, invoice.status, 'sent')) {
      throw new Error(
        `Cannot send invoice. Current status: "${invoice.status}". ` +
        `Allowed transitions: [${(INVOICE_STATUS_TRANSITIONS[invoice.status] || []).join(', ')}]`
      );
    }
    ...
    // Audit log
    await logStatusChange('invoice', invoiceId, invoice.status, 'sent');
  ...
  async markAsPaid(invoiceId: string, currentStatus: string): Promise<void> {
    // Use centralized transition map
    if (!isValidTransition(INVOICE_STATUS_TRANSITIONS, currentStatus, 'paid')) {
      throw new Error(
        `Cannot mark invoice as paid. Current status: "${currentStatus}". ` +
        `Allowed transitions: [${(INVOICE_STATUS_TRANSITIONS[currentStatus] || []).join(', ')}]`
      );
    }
    ...
    // Audit log
    await logStatusChange('invoice', invoiceId, currentStatus, 'paid');
  },
