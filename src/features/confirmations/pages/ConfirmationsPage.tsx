...
import { Plus, Loader2, Clock, ChevronLeft, ChevronRight, CheckCircle2, XCircle } from 'lucide-react';
...
const statusColors: Record<string, { label: string; className: string }> = {
  ...
  confirmed: { label: 'Dikonfirmasi', className: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' },
...
export default function ConfirmationsPage() {
  ...
  const limit = 20;
  const { data, isLoading } = useConfirmations(page, limit, statusFilter || undefined, propertyFilter || undefined);
  ...
  const handleCreate = async (payload: CreateForm) => {
    ...
    } catch { toast({ variant: 'destructive', title: 'Gagal mencatat konfirmasi DP' }); }
  };
  ...
  const handleConfirm = async (payload: ConfirmForm) => {
    ...
    try {
      ...
      setConfirmTarget(null); confirmForm.reset();
      toast({ title: 'Penghuni berhasil dikonfirmasi masuk' });
    } catch { toast({ variant: 'destructive', title: 'Gagal konfirmasi' }); }
  };

  const fmt = (d: string) => { try { return format(new Date(d), 'dd MMM yyyy', { locale: localeId }); } catch { return d; } };
  const sisaHari = (d: string) => { try { return differenceInDays(new Date(d), new Date()); } catch { return null; } };
    ...
    <div className="space-y-5">
          ...
          <Table>
            ...
            <TableBody>
              {confirmations.length === 0 ? (
                <TableRow><TableCell colSpan={7} className="h-32 text-center text-muted-foreground">Belum ada konfirmasi DP.</TableCell></TableRow>
              ) : confirmations.map(c => {
        ...
        <DialogContent className="sm:max-w-lg rounded-2xl">
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <div><DialogTitle>Catat Konfirmasi DP</DialogTitle><p className="text-sm text-muted-foreground">Kamar berstatus available</p></div>
            </div>
          </DialogHeader>
      ...
      <Dialog open={!!confirmTarget} onOpenChange={v => !v && setConfirmTarget(null)}>
        <DialogContent className="sm:max-w-lg rounded-2xl">
          <DialogHeader>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-green-100 flex items-center justify-center"><CheckCircle2 className="h-5 w-5 text-green-600" /></div>
              <div><DialogTitle>Konfirmasi Masuk</DialogTitle>
          ...
          <form onSubmit={confirmForm.handleSubmit(handleConfirm)} className="space-y-4 py-2">
            <div className="space-y-2"><Label>Nama Lengkap</Label><Input defaultValue={confirmTarget?.nama_calon_penghuni} {...confirmForm.register('nama')} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2"><Label>No. Identitas</Label><Input placeholder="3271..." {...confirmForm.register('nomor_identitas')} /></div>
              <div className="space-y-2"><Label>No. Telepon</Label><Input placeholder="0812..." {...confirmForm.register('nomor_telepon')} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2"><Label>Tanggal Masuk</Label><Input type="date" {...confirmForm.register('tanggal_masuk')} /></div>
              <div className="space-y-2"><Label>Durasi (bulan)</Label><Input type="number" min={1} placeholder="6" {...confirmForm.register('durasi_sewa')} /></div>
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setConfirmTarget(null)}>Batal</Button>
              <Button type="submit" disabled={confirmMutation.isPending} className="gap-2 rounded-xl bg-green-600 hover:bg-green-700">
