...
export function TenantForm({ open, onOpenChange, onSubmit, isLoading }: TenantFormProps) {
  ...
  useEffect(() => {
    if (open) reset({ room_id: '', nama: '', nomor_identitas: '', nomor_telepon: '', tanggal_masuk: '', durasi_sewa: 1 });
  }, [open]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg rounded-2xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-2">
          {/* Select property first to filter rooms */}
          <div className="space-y-2">
            <Label>Properti</Label>
            <Select value={selectedProp} onValueChange={(v) => { setSelectedProp(v); setValue('room_id', ''); }}>
          ...
          <div className="space-y-2">
            ...
            <Input id="nama" placeholder="Budi Santoso" disabled={isLoading} {...register('nama')} />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Input id="nomor_identitas" placeholder="3271..." disabled={isLoading} {...register('nomor_identitas')} />
              {errors.nomor_identitas && <p className="text-sm text-destructive">{errors.nomor_identitas.message}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="nomor_telepon">No. Telepon</Label>
              <Input id="nomor_telepon" placeholder="0812..." disabled={isLoading} {...register('nomor_telepon')} />
              {errors.nomor_telepon && <p className="text-sm text-destructive">{errors.nomor_telepon.message}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Input id="tanggal_masuk" type="date" disabled={isLoading} {...register('tanggal_masuk')} />
              {errors.tanggal_masuk && <p className="text-sm text-destructive">{errors.tanggal_masuk.message}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="durasi_sewa">Durasi (bulan)</Label>
              <Input id="durasi_sewa" type="number" min={1} placeholder="6" disabled={isLoading} {...register('durasi_sewa')} />
              {errors.durasi_sewa && <p className="text-sm text-destructive">{errors.durasi_sewa.message}</p>}
            </div>
          </div>
