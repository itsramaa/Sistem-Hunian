...
export function useMerchantUnits(merchantId: string) {
  ...
  const {
    ...
    queryKey: ['merchant-units', merchantId],
    queryFn: () => unitService.fetchMerchantUnits(merchantId),
    enabled: !!merchantId,
  });
  ...
  const createUnitMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['merchant-units', merchantId] });
      // Also invalidate property-specific unit lists
