...
export function useTenants(page = 1, limit = 20, status?: string, property_id?: string, room_id?: string) {
  return useQuery({
    queryKey: [TENANTS_KEY, { page, limit, status, property_id, room_id }],
    queryFn: () => tenantService.list(page, limit, status, property_id, room_id),
  });
}

export function useActiveTenants(page = 1, limit = 20, property_id?: string, room_id?: string) {
  return useTenants(page, limit, 'active', property_id, room_id);
}

export function useTenantHistory(page = 1, limit = 20, property_id?: string, room_id?: string) {
...
export function useCheckoutTenant() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, tanggal_keluar }: { id: string; tanggal_keluar: string }) =>
