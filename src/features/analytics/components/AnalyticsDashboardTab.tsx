...
const RISK_COLORS: Record<string, string> = {
  ...
  rendah: "#22c55e", sedang: "#eab308", tinggi: "#ef4444",
};
...
export function AnalyticsDashboardTab() {
  ...
  const occupancyTrend = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      months.push({ label: format(d, "MMM yy", { locale: idLocale }), start: startOfMonth(d), end: endOfMonth(d) });
    }
  ...
  const riskMapData = useMemo(() => {
    return filtered
      ...
      .map((p) => {
        ...
        return { id: p.id, name: p.name, lat: p.latitude!, lng: p.longitude!, riskLevel: riskLevel.toLowerCase(), score: drp?.overall_risk_score };
      });
  ...
  const tenantQualityData = useMemo(() => {
    ...
    return Object.entries(grades).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value }));
  }, [tenantRisk]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold">Dashboard Analitik</h3>
          <p className="text-sm text-muted-foreground">Visualisasi komprehensif performa properti</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => window.print()} className="print:hidden rounded-xl">
          <Download className="h-4 w-4 mr-2" /> Ekspor PDF
        </Button>
      </div>

      {/* Filters */}
      <Card className="print:hidden rounded-2xl bg-card/90 backdrop-blur-sm border border-border/40">
        <CardContent className="pt-4">
          <div className="flex flex-wrap gap-4 items-end">
            <div className="space-y-1">
              <Label className="text-xs">Tipe Properti</Label>
              <Select value={propertyType} onValueChange={setPropertyType}>
                <SelectTrigger className="w-[160px] h-9 rounded-xl"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1 min-w-[200px]">
              ...
              <Slider min={2020} max={2026} step={1} value={yearRange} onValueChange={setYearRange} />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <Card key={stat.label} className="rounded-2xl bg-card/90 backdrop-blur-sm border border-border/40">
        ...
        <Card className="rounded-2xl bg-card/90 backdrop-blur-sm border border-border/40">
          ...
          <CardContent>
            {riskMapData.length > 0 ? (
                ...
                <MapContainer center={mapCenter} zoom={10} style={{ height: "100%", width: "100%" }} scrollWheelZoom={false}>
                  ...
                  {riskMapData.map((p) => (
                    <CircleMarker key={p.id} center={[p.lat, p.lng]} radius={10}
                      pathOptions={{ color: RISK_COLORS[p.riskLevel] || "#6b7280", fillColor: RISK_COLORS[p.riskLevel] || "#6b7280", fillOpacity: 0.7 }}>
                      <Popup>
                        <div className="text-sm">
                          <p className="font-semibold">{p.name}</p>
                          <p>Risiko: {p.riskLevel}</p>
                          {p.score != null && <p>Skor: {p.score}</p>}
                          <button className="text-primary underline text-xs mt-1" onClick={() => navigate(`/merchant/properties/${p.id}`)}>Lihat Detail →</button>
                        </div>
                      </Popup>
                    </CircleMarker>
                  ))}
              ...
              <div className="h-[300px] flex items-center justify-center text-sm text-muted-foreground">Tidak ada properti dengan koordinat lokasi</div>
            )}
        ...
        <Card className="rounded-2xl bg-card/90 backdrop-blur-sm border border-border/40">
          ...
          <CardContent>
            {tenantQualityData.length > 0 ? (
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie data={tenantQualityData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={({ name, percent }) => `${name} (${(percent * 100).toFixed(0)}%)`}>
                    {tenantQualityData.map((_, i) => <Cell key={i} fill={QUALITY_COLORS[i % QUALITY_COLORS.length]} />)}
                  </Pie>
                  <Tooltip />
                  <Legend />
                </PieChart>
              ...
              <div className="h-[250px] flex items-center justify-center text-sm text-muted-foreground">Belum ada data kualitas tenant</div>
            )}
          </CardContent>
        </Card>
      </div>
