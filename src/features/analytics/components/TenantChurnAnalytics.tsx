...
export function TenantChurnAnalytics() {
  ...
  // Calculate churn rate (churned / (churned + active at period start))
  const churnRate = activeAtPeriodStart.length + recentChurned.length > 0
    ? Math.round((recentChurned.length / (activeAtPeriodStart.length + recentChurned.length)) * 100)
    : 0;
  ...
  const previousChurnRate = activeAtPreviousPeriodStart.length + previousPeriodChurned.length > 0
    ? Math.round((previousPeriodChurned.length / (activeAtPreviousPeriodStart.length + previousPeriodChurned.length)) * 100)
    : 0;
