...
export async function fetchComparativePortfolioData(merchantId: string): Promise<ComparativePortfolioData> {
  const [propertiesRes, unitsRes, paymentsRes, expensesRes] = await Promise.all([
    db.from("properties").select("id, name, address, property_type, total_units, occupied_units").eq("merchant_id", merchantId),
    db.from("units").select("id, property_id, rent_amount, status").eq("merchant_id", merchantId),
    db.from("payments").select("amount, status, created_at").eq("merchant_id", merchantId).eq("status", "paid"),
    db.from("maintenance_expenses").select("total_amount, maintenance_request_id").eq("merchant_id", merchantId),
  ]);
