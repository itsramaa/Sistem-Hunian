...
export default function RoomsPage() {
  ...
  const handleCreate = async (payload: any) => {
    try { await createMutation.mutateAsync(payload); setFormOpen(false); toast({ title: 'Kamar berhasil ditambahkan' }); }
    catch (err: any) {
      const msg = err?.response?.data?.error?.message || err?.response?.data?.message || 'Gagal menambahkan kamar';
      toast({ variant: 'destructive', title: 'Gagal menambahkan kamar', description: msg });
    }
  };
  const handleUpdate = async (payload: any) => {
    if (!editing) return;
    try { await updateMutation.mutateAsync({ id: editing.id, payload }); setEditing(null); setFormOpen(false); toast({ title: 'Kamar berhasil diubah' }); }
    catch { toast({ variant: 'destructive', title: 'Gagal mengubah kamar' }); }
  };
  const handleDelete = async () => {
    if (!deleteTarget) return;
    try { await deleteMutation.mutateAsync(deleteTarget.id); setDeleteTarget(null); toast({ title: 'Kamar berhasil dihapus' }); }
    catch (err: any) { toast({ variant: 'destructive', title: err?.response?.data?.error?.message || 'Gagal menghapus kamar' }); setDeleteTarget(null); }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3 overflow-x-auto pb-1">
        <div className="relative shrink-0">
          ...
          <Input placeholder="Cari nomor kamar..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9 rounded-xl h-10 w-[180px]" />
        </div>
            ...
            return (
              <DataCard key={room.id}
                header={
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-semibold text-sm">{room.nomor_kamar}</p>
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium mt-0.5 ${sc.className}`}>{sc.label}</span>
                      </div>
                    </div>
                ...
                fields={[
                  ...
                  { label: 'Harga Sewa', value: `Rp${room.harga_sewa.toLocaleString('id-ID')}` },
                ...
                {rooms.map((room: Room) => {
                  ...
                  return (
                    <TableRow key={room.id} className="group hover:bg-primary/5 transition-colors">
                      ...
                      <TableCell className="text-right"><RoomActions room={room} /></TableCell>
                    </TableRow>
                  );
                })}
              </TableBody>
      ...
      <RoomForm open={formOpen} onOpenChange={setFormOpen} room={editing} onSubmit={editing ? handleUpdate : handleCreate} isLoading={createMutation.isPending || updateMutation.isPending} />
        ...
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <div><DialogTitle>Hapus Kamar</DialogTitle><p className="text-sm text-muted-foreground mt-0.5">Kamar <strong>{deleteTarget?.nomor_kamar}</strong> akan dihapus.</p></div>
            </div>
          </DialogHeader>
