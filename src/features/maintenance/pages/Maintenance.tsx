...
export default function MaintenancePage() {
  ...
  const createForm = useForm<CreateForm>({ resolver: zodResolver(createSchema), defaultValues: { room_id: '', tanggal_laporan: today, deskripsi_kerusakan: '' } });
  const updateForm = useForm<UpdateForm>({ resolver: zodResolver(updateSchema), defaultValues: { tindakan_penanganan: '', biaya: 0, status: 'reported' } });

  const handleCreate = async (payload: CreateForm) => {
    try { await createMutation.mutateAsync(payload); setCreateOpen(false); createForm.reset({ tanggal_laporan: today }); toast({ title: 'Laporan maintenance berhasil dicatat' }); }
    catch { toast({ variant: 'destructive', title: 'Gagal mencatat maintenance' }); }
  };
  const handleUpdate = async (payload: UpdateForm) => {
    if (!updateTarget) return;
    try { await updateMutation.mutateAsync({ id: updateTarget.id, payload }); setUpdateTarget(null); toast({ title: 'Progress maintenance berhasil diupdate' }); }
    catch { toast({ variant: 'destructive', title: 'Gagal update maintenance' }); }
  };
  const openUpdate = (m: Maintenance) => { setUpdateTarget(m); updateForm.reset({ tindakan_penanganan: m.tindakan_penanganan || '', biaya: m.biaya || 0, status: m.status }); };
  ...
  return (
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Wrench} title="Belum ada laporan maintenance" description="Buat laporan kerusakan kamar baru." action={{ label: 'Buat Laporan', onClick: () => setCreateOpen(true), icon: Plus }} />
      ) : isMobile ? (
        /* Mobile: Card view */
              ...
              <DataCard key={m.id}
                header={
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-sm">{m.nomor_kamar || '—'} · {m.nama_properti || '—'}</p>
                      <p className="text-xs text-muted-foreground truncate max-w-[200px]">{m.deskripsi_kerusakan}</p>
                    </div>
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium shrink-0 ${sc.className}`}>{sc.label}</span>
                  </div>
                }
                fields={[
                  { label: 'Tgl Laporan', value: fmt(m.tanggal_laporan) },
                  { label: 'Biaya', value: m.biaya ? `Rp${m.biaya.toLocaleString('id-ID')}` : undefined },
                ]}
                actions={m.status !== 'completed' ? (
                  <Button variant="ghost" size="sm" className="h-9 gap-1.5 text-xs rounded-lg min-h-[44px]" onClick={() => openUpdate(m)}>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-lg rounded-2xl">
          ...
          <form onSubmit={createForm.handleSubmit(handleCreate)} className="space-y-4 py-2">
            ...
            <div className="space-y-2"><Label>Tanggal Laporan</Label><Input type="date" {...createForm.register('tanggal_laporan')} /></div>
            <div className="space-y-2">
        ...
        <DialogContent className="sm:max-w-lg rounded-2xl">
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <div><DialogTitle>Update Progress</DialogTitle><p className="text-sm text-muted-foreground">{updateTarget?.nomor_kamar} — {updateTarget?.deskripsi_kerusakan?.slice(0, 40)}</p></div>
            </div>
          </DialogHeader>
          <form onSubmit={updateForm.handleSubmit(handleUpdate)} className="space-y-4 py-2">
            ...
            <div className="space-y-2"><Label>Tindakan Penanganan (opsional)</Label><Textarea placeholder="Tambal dengan sealant waterproof..." rows={2} {...updateForm.register('tindakan_penanganan')} /></div>
            <div className="space-y-2"><Label>Biaya (Rp, opsional)</Label><Input type="number" min={0} placeholder="250000" {...updateForm.register('biaya')} /></div>
            <DialogFooter>
