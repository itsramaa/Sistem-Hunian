...
const OCR_TYPES = [
  ...
  { id: "maintenance_receipt", title: "Nota Pemeliharaan", description: "Ekstrak item, total, dan vendor dari nota/struk untuk pencatatan biaya pemeliharaan.", icon: Wrench, tier: "Professional", avgTime: "2-3 detik", fields: ["Vendor", "Item", "Qty", "Harga", "Total", "Tanggal"], iconBg: "from-warning/20 to-warning/5" },
];
...
function OcrTutorial() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader icon={ScanText} title="Panduan OCR" description="Pelajari cara menggunakan OCR untuk memproses dokumen secara otomatis" />

      {/* Progress Bar */}
      <div className="space-y-2" role="region" aria-label="Progres Panduan">
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">Progres Panduan</span>
          <span className="font-semibold">{Math.round(progress)}%</span>
        </div>
        <Progress value={progress} className="h-2.5 rounded-full [&>div]:bg-gradient-to-r [&>div]:from-primary [&>div]:to-primary/70" aria-label={`Progres panduan: ${Math.round(progress)}%`} />
      </div>

      {/* Step Navigator */}
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2" role="tablist" aria-label="Langkah-langkah panduan">
      ...
      {/* Step Content */}
      <div role="tabpanel" aria-labelledby={`step-${currentStep + 1}`}>
        {currentStep === 0 && (
          <div className="space-y-4">
            <Card className="rounded-2xl bg-card/90 backdrop-blur-sm border border-border/40">
              <CardHeader>
                <CardTitle className="flex items-center gap-2.5" id="step-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {OCR_TYPES.map((type) => (
                <Card
                  ...
                  aria-label={`Pilih tipe ${type.title}`}
                >
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-base flex items-center gap-2.5">
                        <div className={`h-9 w-9 rounded-xl bg-gradient-to-br ${type.iconBg} flex items-center justify-center`} aria-hidden="true">
        ...
        {currentStep === 1 && (
          <Card className="rounded-2xl bg-card/90 backdrop-blur-sm border border-border/40">
            <CardHeader>
              <CardTitle className="flex items-center gap-2.5" id="step-2">
                <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center" aria-hidden="true">
                  <Upload className="h-4.5 w-4.5 text-primary" />
                </div>
                Langkah 2: Unggah Dokumen
              </CardTitle>
              <CardDescription>Unggah foto atau hasil pindai dokumen Anda</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="border-2 border-dashed rounded-2xl p-12 text-center space-y-4 bg-background/40 backdrop-blur-sm border-border/50 hover:border-primary/50 transition-colors" role="presentation">
                <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-muted/80 to-muted/30 flex items-center justify-center mx-auto" aria-hidden="true">
                  <Upload className="h-7 w-7 text-muted-foreground" />
                </div>
                <div>
                  <p className="font-semibold">Tarik & lepas atau klik untuk unggah</p>
                  <p className="text-sm text-muted-foreground">Format: JPG, PNG, PDF (maks 10MB)</p>
                </div>
              ...
              <div className="bg-muted/30 backdrop-blur-sm rounded-xl p-4 space-y-2 border border-border/30">
                ...
                <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
                  <li>Pastikan dokumen dalam kondisi terang dan tidak kabur</li>
        ...
        {currentStep === 2 && (
          <Card className="rounded-2xl bg-card/90 backdrop-blur-sm border border-border/40">
            <CardHeader>
              <CardTitle className="flex items-center gap-2.5" id="step-3">
                <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center" aria-hidden="true">
            ...
            <CardContent className="space-y-6">
              <div className="bg-card/80 backdrop-blur-sm rounded-xl p-6 space-y-4 border border-border/40">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">AI mengidentifikasi dan mengekstrak kolom data</p>
                  </div>
                  <Badge variant="outline" className="rounded-full">2-4 detik</Badge>
                </div>
                <div className="space-y-2" role="list">
                  {["Deteksi teks", "Klasifikasi kolom", "Validasi data", "Skor kepercayaan"].map((step) => (
                    <div key={step} className="flex justify-between text-sm items-center" role="listitem">
                      <span>{step}</span>
                      <div className="h-5 w-5 rounded-full bg-success/15 flex items-center justify-center" aria-hidden="true">
              ...
              <div className="bg-primary/5 rounded-xl p-4 border border-primary/20">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  AI Vision menganalisis gambar dokumen, mengidentifikasi area teks, mengekstrak informasi terstruktur, dan memberikan skor kepercayaan (confidence) untuk setiap kolom. Jika kepercayaan &lt; 80%, sistem akan meminta peninjauan manual.
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        {currentStep === 3 && (
          <Card className="rounded-2xl bg-card/90 backdrop-blur-sm border border-border/40">
            <CardHeader>
              <CardTitle className="flex items-center gap-2.5" id="step-4">
                ...
                Langkah 4: Tinjau & Konfirmasi
              </CardTitle>
              <CardDescription>Tinjau hasil ekstraksi AI dan konfirmasi data</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="rounded-xl overflow-hidden border border-border/40" role="table" aria-label="Contoh Hasil Ekstraksi">
                {[
                  ...
                  <div key={field.label} className={`flex items-center justify-between p-3 ${idx > 0 ? 'border-t border-border/30' : ''} bg-card/60 backdrop-blur-sm`} role="row">
                    <div role="cell">
                      <p className="text-sm font-semibold">{field.label}</p>
                      <p className="text-sm text-muted-foreground">{field.value}</p>
                    </div>
                    <Badge variant={field.confidence >= 80 ? "default" : "secondary"} className="rounded-full" role="cell">
                      {field.confidence}%
                    </Badge>
                  </div>
                ))}
              ...
              <div className="flex items-center gap-2 p-3 bg-warning/10 rounded-xl border border-warning/20" role="alert">
                <Eye className="h-4 w-4 text-warning shrink-0" aria-hidden="true" />
                <p className="text-sm text-warning">
                  Kolom "Alamat" memiliki tingkat kepercayaan rendah (75%). Harap periksa dan koreksi jika perlu.
                </p>
              </div>
              ...
              <p className="text-xs text-muted-foreground text-center">
                Ini adalah demo panduan. Untuk menggunakan OCR, buka halaman terkait (Penyewa, Pembayaran, Pemeliharaan) dan gunakan tombol Unggah Dokumen.
              </p>
            </CardContent>
          </Card>
        )}
