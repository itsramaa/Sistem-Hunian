...
export default function Inventory() {
  ...
  const { data: facilityTypes = [], isLoading: loadingTypes } = useQuery({
    ...
    queryFn: async () => {
      // TODO: Go endpoint not yet implemented — was: supabase.from('facility_types').select('*').eq('merchant_id', merchantId)
      return [];
    },
    enabled: !!merchantId,
  });
  ...
  const { data: assets = [], isLoading: loadingAssets } = useQuery({
    ...
    queryFn: async () => {
      // TODO: Go endpoint not yet implemented — was: supabase.from('assets').select('*, facility_type:facility_types(name, asset_type, scope), property:properties(name), unit:units(unit_number)').eq('merchant_id', merchantId)
      return [];
    },
    enabled: !!merchantId,
  });
  ...
  const { data: assignments = [], isLoading: loadingAssignments } = useQuery({
    ...
    queryFn: async () => {
      // TODO: Go endpoint not yet implemented — was: supabase.from('facility_assignments').select('*, facility_type:facility_types(name, merchant_id), property:properties(name), unit:units(unit_number)')
      return [];
    },
    enabled: !!merchantId,
  });
