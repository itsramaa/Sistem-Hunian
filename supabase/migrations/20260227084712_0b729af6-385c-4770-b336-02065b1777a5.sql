...
-- Remove escrow account auto-creation from handle_new_user()
...
BEGIN
    -- Get role from metadata, default to merchant
    user_role := COALESCE(
    ...
    IF user_role = 'merchant' THEN
        ...
        -- NOTE: Escrow account creation removed — using direct payment model
...
-- Drop create_merchant_escrow trigger if it exists
DROP TRIGGER IF EXISTS create_merchant_escrow ON public.merchants;
DROP FUNCTION IF EXISTS public.create_merchant_escrow();
