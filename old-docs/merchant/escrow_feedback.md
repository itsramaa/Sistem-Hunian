...
| ESC-B03 | High | ON_DEMAND_FEE_RATE hardcoded (0.5%) may not match actual Xendit fees | Constants | Fetch from config | ✅ Fixed |
| ESC-B04 | High | Disbursement options show different fees than what may be charged | DISBURSEMENT_OPTIONS | Sync with actual fee structure | ✅ Fixed |
| ESC-B05 | Medium | Single() query for escrow_account may throw if not exists | escrowAccount query | Use maybeSingle() | ✅ Fixed |

### Validations
| ID | Severity | Issue | Location | Recommendation | Status |
|----|----------|-------|----------|----------------|--------|
| ESC-V01 | High | Minimum disbursement amount from database but not always available | requestDisbursement | Handle missing min_disbursement_amount | ✅ Fixed |
| ESC-V02 | High | No validation that balance hasn't changed during request | requestDisbursement | Add optimistic locking | - |
| ESC-V03 | Medium | Bank account existence checked but not validated | requestDisbursement | Verify account details | ✅ Fixed |

### UX & Flow Pengguna
| ID | Severity | Issue | Location | Recommendation | Status |
|----|----------|-------|----------|----------------|--------|
| ESC-U01 | High | No bank account management visible on this page | Escrow.tsx | Add link to add bank account | ✅ Fixed |
| ESC-U02 | Medium | Transaction history limited to 50 without pagination | transactions query | Add pagination/load more | ✅ Fixed |
...
| ID | Severity | Issue | Location | Recommendation | Status |
|----|----------|-------|----------|----------------|--------|
| ESC-S01 | Critical | Disbursement can be requested without proper bank verification | requestDisbursement | Verify bank account before disbursement | ✅ Fixed |
...
| ID | Severity | Issue | Location | Recommendation | Status |
|----|----------|-------|----------|----------------|--------|
| ESC-E01 | High | Generic error messages for disbursement failures | requestDisbursement | Provide specific error context | ✅ Fixed |
...
| ESC-M02 | Medium | Fee calculation duplicated | Multiple places | Centralize fee logic | ✅ Fixed |
| ESC-M03 | Medium | Disbursement options hardcoded | DISBURSEMENT_OPTIONS | Move to config/database | - |
| ESC-M04 | Low | Status badge styling inline | getStatusBadge | Use shared component | ✅ Fixed |

### Compatibility & Environment
| ID | Severity | Issue | Location | Recommendation | Status |
|----|----------|-------|----------|----------------|--------|
| ESC-X01 | Low | Currency formatting locale hardcoded | formatCurrency | Make configurable | ✅ Fixed |
