...
|----|----------|-------|----------|----------------|
| MNT-B01 | Critical | Vendor job created without checking if vendor accepts category | updateMutation | Validate vendor service_categories match request category |
| MNT-B02 | High | assigned_to stores business_name string, not vendor_id reference | updateMutation | Use assigned_vendor_id FK properly |
| MNT-B03 | High | SLA badge uses type assertion without validation | SLABadge component | Properly type maintenance request |
| MNT-B04 | Medium | Duplicate vendor_job created if update called multiple times | updateMutation | Check for existing job before creating |
| MNT-B05 | Medium | Completed status set without completion notes/photos | updateMutation | Require completion details for completed status |

### Validations
| ID | Severity | Issue | Location | Recommendation |
|----|----------|-------|----------|----------------|
| MNT-V01 | High | Agreed price can be zero or negative | Update dialog | Add min validation for agreed_price |
...
| MNT-V03 | Medium | Status can transition backwards (e.g., completed to pending) | Update dialog | Add status transition rules |

### UX & Flow Pengguna
| ID | Severity | Issue | Location | Recommendation |
|----|----------|-------|----------|----------------|
| MNT-U01 | High | Update dialog lacks context (photos, history) | Update dialog | Show request details in dialog |
...
| ID | Severity | Issue | Location | Recommendation |
|----|----------|-------|----------|----------------|
| MNT-P01 | Medium | All verified vendors fetched regardless of category | vendors query | Filter vendors by relevant categories |
...
| ID | Severity | Issue | Location | Recommendation |
|----|----------|-------|----------|----------------|
| MNT-C01 | High | assigned_to (string) and assigned_vendor_id (FK) not synchronized | updateMutation | Use only FK, derive display name |
| MNT-C02 | High | Vendor job created but request status may not update atomically | updateMutation | Use database transaction |
| MNT-C03 | Medium | resolved_at set but completion details not required | updateMutation | Require notes/photos for completion |
...
| MNT-E02 | Medium | Generic error messages for updates | updateMutation | Provide specific error context |

### Maintainability
| ID | Severity | Issue | Location | Recommendation |
|----|----------|-------|----------|----------------|
| MNT-M01 | Medium | Status and priority color logic inline | Maintenance.tsx | Extract to shared utility |
