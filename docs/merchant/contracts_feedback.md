...
| CON-B04 | Medium | End date validation doesn't prevent past dates | contractSchema | Add refinement to ensure end_date > today | ✅ Fixed |
| CON-B05 | Medium | Contract can be created with end_date before start_date | contractSchema | Add cross-field validation | ✅ Fixed |

### Validations
| ID | Severity | Issue | Location | Recommendation | Status |
|----|----------|-------|----------|----------------|--------|
| CON-V01 | High | No validation that unit isn't already under active contract | handleCreateContract | Check for existing active contracts on unit | ✅ Fixed |
| CON-V02 | High | No validation that tenant isn't already on another active contract | handleCreateContract | Check for existing active contracts for tenant | ✅ Fixed |
| CON-V03 | Medium | Terms field has no length limit in UI | contractSchema | Add max length validation | ✅ Fixed |
| CON-V04 | Medium | Deposit amount can be negative | contractSchema | Add min(0) validation | ✅ Fixed |
...
| CON-S04 | Medium | Contract terms can be edited after tenant signs | handleSaveTerms | Lock terms after tenant signature | ✅ Fixed |
...
| CON-E02 | Medium | Generic error messages for contract operations | Multiple mutations | Provide specific error context | ✅ Fixed |
