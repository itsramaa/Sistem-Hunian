...
|----|----------|-------|----------|----------------|
| INV-B01 | Critical | Invoice status transitions not validated (e.g., can mark cancelled invoice as paid) | markPaidMutation | Add status transition validation |
...
| INV-B04 | Medium | Tax amount can be negative | createMutation | Validate non-negative tax |
| INV-B05 | Medium | Due date can be in past | Create Invoice form | Validate due_date >= today |

### Validations
| ID | Severity | Issue | Location | Recommendation |
|----|----------|-------|----------|----------------|
| INV-V01 | High | No validation for duplicate invoices (same contract, same period) | createMutation | Check for existing invoice for period |
| INV-V02 | High | Amount field allows zero or negative values | Create Invoice form | Add min validation |
| INV-V03 | Medium | Description has no length limit | Create Invoice form | Add max length validation |
